...
function moondental_seo_region_data( $key ) {
	...
	return array(
		'title' => sprintf( '%s 치과 추천 · 임플란트·충치·잇몸 | 문치과병원 %d분', $name, $duration ), // v3.98 · 페이지 이름 먼저
		'desc'  => sprintf(
...
/**
 * v3.95 · 홈 검색 제목·설명 확정값 1회 반영
 *   Customizer 에 저장된 옛 값이 파일 기본값보다 우선하므로 theme_mod 를 직접 덮어쓴다.
 *   이후 관리자가 Customizer 에서 수정하면 그 값이 유지된다(플래그로 1회만 실행).
 */
add_action( 'wp_loaded', function () {
	if ( get_option( 'md_seo_home_v3955' ) === 'done' ) return;
	set_theme_mod( 'md_content_seo_home_title', '한아의료재단 문치과병원 | 천안 아산에서 30여년을 한결같이' );
	set_theme_mod( 'md_content_seo_home_desc',  '한아의료재단 문치과병원은 4개 전문센터와 분과별 협진 시스템을 운영하고 있는 천안 아산의 종합 치과병원입니다. 임플란트·교정·라미네이트·매복사랑니·턱관절.' );
	update_option( 'md_seo_home_v3955', 'done' );
}, 60 );

// v3.98 · 하위 페이지 제목을 「페이지 이름 먼저 | 문치과병원」 으로 통일 (사이트링크·SERP 잘림 대응) — Customizer 값을 맵 제목으로 덮어쓴다
add_action( 'wp_loaded', function () {
	if ( get_option( 'md_seo_titles_v398' ) === 'done' ) return;
	$map  = moondental_seo_page_map();
	$keys = moondental_seo_customizer_key_map();
	foreach ( $keys as $slug => $ck ) {
		if ( $slug === '_home' || empty( $map[ $slug ]['title'] ) ) continue;
		set_theme_mod( 'md_content_seo_' . $ck . '_title', $map[ $slug ]['title'] );
	}
	update_option( 'md_seo_titles_v398', 'done' );
}, 61 );
