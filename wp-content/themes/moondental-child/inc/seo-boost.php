...
/* ============================================================
 * v3.44.206 · AI 검색(생성형 답변 엔진) 대응
 *   답변 엔진은 (1) 기계가 읽을 수 있는 구조, (2) 질문에 직접 답하는 문단,
 *   (3) 개체(entity) 식별이 명확한 사이트를 인용한다. 아래는 그 셋을 보강한다.
 * ========================================================== */

/**
 * FAQPage JSON-LD · 진료항목 페이지와 FAQ 페이지
 *   질문·답변 쌍은 답변 엔진이 그대로 인용하기 가장 쉬운 형태다.
 */
function moondental_jsonld_faq() {
	$pairs = array();

	// 1) 진료항목 페이지 — slug 별 FAQ
	if ( is_page() && function_exists( 'moondental_get_faqs_by_service' ) ) {
	...
	// 2) FAQ 페이지 — 카테고리별 전체
	if ( empty( $pairs ) && is_page_template( 'page-templates/page-faq.php' ) && function_exists( 'moondental_get_faqs' ) ) {
		foreach ( (array) moondental_get_faqs() as $items ) {
			foreach ( (array) $items as $item ) $pairs[] = $item;
		}
	}
...
add_action( 'wp_head', 'moondental_jsonld_faq', 52 );
 ...
 */
add_action( 'init', function () {
	$path = strtok( isset( $_SERVER['REQUEST_URI'] ) ? wp_unslash( $_SERVER['REQUEST_URI'] ) : '', '?' );
