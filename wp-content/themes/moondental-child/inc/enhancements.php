...
function moondental_floor_guide_data() {
	return array(
		...
		array(
			'floor'   => '11F',
			'centers' => array(
				array( 'name' => '교정센터',   'slug' => '투명교정-센터', 'center' => 'suresmile' ),
	...
	foreach ( $data as $f ) {
		...
		foreach ( $f['centers'] as $c ) {
			...
			if ( $args['link_pages'] && ! empty( $c['center'] ) && ! empty( $c['slug'] ) ) {
				...
				$parts[] = '<span class="md-floor-guide__center' . $color_cls . '">' . $name . '</span>';
			}
		}
...
function moondental_get_testimonials() {
	$defaults = array(
		...
		array(
			...
			'service' => '자연치아 살리기',
			'rating'  => 5,
			'text'    => '발치하고 임플란트 하라던 치아를 보존과 원장님이 살려주셨어요. 재근관치료로 자연치를 지킬 수 있어서 정말 감사합니다.',
		),
		array(
			'name'    => '한○○',
			'gender'  => '여성',
		...
		'임플란트-센터' => array(
			...
			array( 'q' => '비용이 어느 정도 들까요?',
				   'a' => '일반 임플란트 120~150만 원, 프리미엄(메가젠 블루다이아몬드) 180~230만 원 선입니다. 정확한 비용은 정밀 진단 후 안내드리며, ' .
				   	  '<a href="' . home_url( '/비용-안내/' ) . '">비급여 진료비 안내</a>에서 전체 가격대를 확인하실 수 있습니다.' ),
		),
		...
		'투명교정-센터' => array(
			...
			array( 'q' => '비용은 얼마나 드나요?',
				   'a' => '메탈 브라켓 400~600만 원, 투명교정 600~1,000만 원, 설측 1,000~1,500만 원 선입니다. ' .
				   	  '<a href="' . home_url( '/비용-안내/' ) . '">비급여 진료비 안내</a>에서 자세히 보실 수 있습니다.' ),
		),
...
function moondental_service_pain_points() {
	$result = array(
		'임플란트-센터' => array(
			...
			array( 'concern' => '비용이 부담돼요',                       'solution' => '진단 후 투명한 비용 설명을 먼저 드립니다. 카드 무이자 할부 가능, 만 65세 이상은 건강보험도 안내드립니다.' ),
		...
		'투명교정-센터' => array(
			...
			array( 'concern' => '투명교정 vs 일반교정 뭐가 좋나요?',     'solution' => '라이프스타일·난이도에 맞춰 교정 진료팀이 최적안을 추천드립니다.' ),
		...
		'자연치아-살리기' => array(
			...
			array( 'concern' => '재근관치료 가능한가요?',                'solution' => '현미경 정밀 진료로 약 70~80% 성공률을 보입니다. 보존과 진료팀이 직접 진료합니다.' ),
			...
			array( 'concern' => '다른 치과 신경치료가 실패했어요',       'solution' => '보존과 진료팀의 재근관치료 + 치근단 수술로 단계적 접근이 가능합니다.' ),
		),
		'턱관절-클리닉' => array(
...
function moondental_get_faqs() {
	return array(
		...
		'예약·내원' => array(
			...
			array( 'q' => '주차가 가능한가요?',
				   'a' => '병원 지하 기계식 주차장(천안시 동남구 만남로 52 문타워)을 무료로 이용하실 수 있습니다. 기계식이라 SUV·대형차는 인근 신부 제5공영주차장(천안시 동남구 먹거리1길 10, 도보 5분)에 주차해주세요. 주차 후 병원 접수처에서 주차도장/주차권을 받아가세요.' ),
