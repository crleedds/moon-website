...
/**
 * v3.44.214 · 백과사전 확장 6차 · 26개 신규 용어 APPEND
 *   기존 602개 유지 · 중복 title 은 건너뛰고 신규만 insert.
 *   중점: 치과 재료학 · 감염관리 · 디지털 치의학 심화 · 진단 영상
 */
function moondental_seed_encyclopedia_v344214() {
	if ( get_option( 'moondental_encyclopedia_seed_v344214' ) === 'done' ) return;
	$seed_file = MOONDENTAL_DIR . '/inc/encyclopedia-seed-v344214.php';
	if ( ! file_exists( $seed_file ) ) return;
	...
	update_option( 'moondental_encyclopedia_seed_v344214', 'done' );
	update_option( 'moondental_encyclopedia_seed_v344214_count', $inserted, false );
	update_option( 'moondental_encyclopedia_seed_v344214_skipped', $skipped_dup, false );
}
add_action( 'admin_init', 'moondental_seed_encyclopedia_v344214', 68 );
add_action( 'wp_loaded',  'moondental_seed_encyclopedia_v344214', 68 );

/**
 * v3.90 · 백과사전 확장 · 90개 신규 용어 APPEND (encyclopedia-seed-v390.php)
 *   임플란트·교정·심미보철·충치/잇몸·구강외과/턱관절·소아·치과상식
 *   기존 항목 유지, title 중복은 건너뜀
 */
function moondental_seed_encyclopedia_v390() {
	if ( get_option( 'moondental_encyclopedia_seed_v390' ) === 'done' ) return;
	$seed_file = MOONDENTAL_DIR . '/inc/encyclopedia-seed-v390.php';
	if ( ! file_exists( $seed_file ) ) return;
	// v3.90.1 · 동시 요청에서 두 번 실행되어 중복 삽입되는 것을 막는 잠금 (add_option 은 이름 중복 시 실패)
	if ( ! add_option( 'moondental_encyclopedia_seed_v390_lock', (string) time(), '', 'no' ) ) return;
	...
	update_option( 'moondental_encyclopedia_seed_v390', 'done' );
	update_option( 'moondental_encyclopedia_seed_v390_count', $inserted, false );
	update_option( 'moondental_encyclopedia_seed_v390_skipped', $skipped_dup, false );
	delete_option( 'moondental_encyclopedia_seed_v390_lock' );
}
add_action( 'admin_init', 'moondental_seed_encyclopedia_v390', 69 );
add_action( 'wp_loaded',  'moondental_seed_encyclopedia_v390', 69 );

/**
 * v3.90.1 · md_term 제목 중복 정리
 *   시더가 두 요청에서 동시에 실행되어 같은 제목이 두 번 삽입된 경우(75건) +
 *   초기 시드 자체의 중복(6건)을 정리한다. 먼저 만들어진 글(낮은 ID · 원래 슬러그)을 남기고 나머지를 영구 삭제.
 *   add_option 잠금으로 동시 실행을 막는다.
 */
function moondental_dedupe_md_terms_v3901() {
	if ( get_option( 'moondental_md_term_dedupe_v3901' ) === 'done' ) return;
	if ( ! add_option( 'moondental_md_term_dedupe_v3901_lock', (string) time(), '', 'no' ) ) return; // 다른 요청이 실행 중

	global $wpdb;
	$rows = $wpdb->get_results( "SELECT ID, post_title FROM {$wpdb->posts} WHERE post_type = 'md_term' AND post_status = 'publish' ORDER BY ID ASC" );
	$seen = array();
	$removed = 0;
	foreach ( (array) $rows as $r ) {
		...
		if ( $t === '' ) continue;
		if ( isset( $seen[ $t ] ) ) {
			if ( wp_delete_post( (int) $r->ID, true ) ) $removed++;
			continue;
		}
		$seen[ $t ] = true;
	}
	...
	update_option( 'moondental_md_term_dedupe_v3901', 'done' );
	update_option( 'moondental_md_term_dedupe_v3901_count', $removed, false );
	delete_option( 'moondental_md_term_dedupe_v3901_lock' );
}
add_action( 'admin_init', 'moondental_dedupe_md_terms_v3901', 70 );
add_action( 'wp_loaded',  'moondental_dedupe_md_terms_v3901', 70 );

/**
 * v3.91 · 백과사전 확장 · 증상 중심 38개 APPEND (encyclopedia-seed-v391.php)
 *   환자가 검색하는 증상·상황 표현 (이가 흔들려요 · 단 것에 아파요 …)
 *   기존 항목 유지, title 중복은 건너뜀
 */
function moondental_seed_encyclopedia_v391() {
	if ( get_option( 'moondental_encyclopedia_seed_v391' ) === 'done' ) return;
	$seed_file = MOONDENTAL_DIR . '/inc/encyclopedia-seed-v391.php';
	if ( ! file_exists( $seed_file ) ) return;
	if ( ! add_option( 'moondental_encyclopedia_seed_v391_lock', (string) time(), '', 'no' ) ) return; // 동시 실행 방지
	...
	update_option( 'moondental_encyclopedia_seed_v391', 'done' );
	update_option( 'moondental_encyclopedia_seed_v391_count', $inserted, false );
	update_option( 'moondental_encyclopedia_seed_v391_skipped', $skipped_dup, false );
	delete_option( 'moondental_encyclopedia_seed_v391_lock' );
}
add_action( 'admin_init', 'moondental_seed_encyclopedia_v391', 71 );
add_action( 'wp_loaded',  'moondental_seed_encyclopedia_v391', 71 );

/**
 * v3.92 · 백과사전 확장 · 구조·해부 기초 + 세부 용어 + 환자 질문형 59개 APPEND (encyclopedia-seed-v392.php)
 *   법랑질·상아질·치수·치주인대 … · 티타늄·2차 수술·맞춤 지대주 · 페룰·교합 조정 · 유지장치 기간 등
 *   기존 항목 유지, title 중복은 건너뜀
 */
function moondental_seed_encyclopedia_v392() {
	if ( get_option( 'moondental_encyclopedia_seed_v392' ) === 'done' ) return;
	$seed_file = MOONDENTAL_DIR . '/inc/encyclopedia-seed-v392.php';
	if ( ! file_exists( $seed_file ) ) return;
	if ( ! add_option( 'moondental_encyclopedia_seed_v392_lock', (string) time(), '', 'no' ) ) return; // 동시 실행 방지
	...
	update_option( 'moondental_encyclopedia_seed_v392', 'done' );
	update_option( 'moondental_encyclopedia_seed_v392_count', $inserted, false );
	update_option( 'moondental_encyclopedia_seed_v392_skipped', $skipped_dup, false );
	delete_option( 'moondental_encyclopedia_seed_v392_lock' );
}
add_action( 'admin_init', 'moondental_seed_encyclopedia_v392', 69 );
add_action( 'wp_loaded',  'moondental_seed_encyclopedia_v392', 69 );

/**
 * v3.93 · 백과사전 확장 · 구강내과·재료·보험·전신질환 42개 APPEND (encyclopedia-seed-v393.php)
 *   입안 궤양 2주·OAS·약물 부작용 · 시멘트·PMMA·PEEK·레이저 · 레진 급여·잇몸 급여·본인부담률 · 고혈압·뇌졸중·치매·파킨슨·천식·뇌전증 등
 *   기존 항목 유지, title 중복은 건너뜀
 */
function moondental_seed_encyclopedia_v393() {
	if ( get_option( 'moondental_encyclopedia_seed_v393' ) === 'done' ) return;
	$seed_file = MOONDENTAL_DIR . '/inc/encyclopedia-seed-v393.php';
	if ( ! file_exists( $seed_file ) ) return;
	if ( ! add_option( 'moondental_encyclopedia_seed_v393_lock', (string) time(), '', 'no' ) ) return; // 동시 실행 방지
	...
	update_option( 'moondental_encyclopedia_seed_v393', 'done' );
	update_option( 'moondental_encyclopedia_seed_v393_count', $inserted, false );
	update_option( 'moondental_encyclopedia_seed_v393_skipped', $skipped_dup, false );
	delete_option( 'moondental_encyclopedia_seed_v393_lock' );
}
add_action( 'admin_init', 'moondental_seed_encyclopedia_v393', 73 );
add_action( 'wp_loaded',  'moondental_seed_encyclopedia_v393', 73 );
