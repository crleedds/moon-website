			...
			<div class="md-header__inner">
				...
				<a class="md-header__brand" href="<?php echo esc_url( home_url( '/' ) ); ?>" aria-label="<?php echo esc_attr( moondental_get_info( 'name_full' ) ); ?>">
					<?php if ( has_custom_logo() ) : ?>
						<?php the_custom_logo(); ?>
					<?php else :
						// Light/dark 변형 모두 준비 — picture로 다크모드 자동 전환
						$logo_light = '';
						...
						if ( $logo_light ) : ?>
							<picture>
								<?php if ( $logo_dark ) : ?>
									<source srcset="<?php echo esc_url( $logo_dark ); ?>" media="(prefers-color-scheme: dark)">
								<?php endif; ?>
								<img class="md-header__brand-img" src="<?php echo esc_url( $logo_light ); ?>"
