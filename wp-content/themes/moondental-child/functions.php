...
define( 'MOONDENTAL_VERSION', '4.6.5' );
...
/* v3.44.188 · trust_1_value · 저장된 값이 옛날 형태이면 삭제해서 동적 계산값(현재-1995) 사용 */
add_action( 'init', function() {
	if ( get_option( 'moondental_trust1_v34188' ) === 'done' ) return;
	$stored = get_theme_mod( 'md_content_trust_1_value', null );
	if ( in_array( $stored, array( '30', '30년', '30여', '30여년' ), true ) ) {
...
<p>야간 진료도 예약제로 운영되니, 방문 전 네이버 예약·전화(041-563-2875)·카카오톡 채널로 시간 확인 부탁드립니다.</p>" ),
		...
		array( 'cat'=>'notice', 'title'=>'무료 주차 안내 — 병원 지하 · 신부 제5공영주차장',
			'body'=>"<p>문치과병원 방문 시 무료 주차를 이용하실 수 있습니다.</p>
<h3>병원 지하 기계식 주차장 (SUV 불가)</h3>
<p>천안시 동남구 만남로 52 문타워. 승용차 주차가 가능하며, 기계식이라 SUV·대형차는 이용이 어렵습니다.</p>
<h3>신부 제5공영주차장 (SUV 가능)</h3>
<p>천안시 동남구 먹거리1길 10 (도보 5분).</p>
<p>주차 후 병원 접수처에서 주차도장/주차권을 받아가세요.</p>
 ...
 * @return mixed
 */
/** v3.99.4 · Customizer 에 저장된 주소에서 「(신부동)」 제거 (1회) */
add_action( 'wp_loaded', function () {
	if ( get_option( 'md_addr_sinbu_v3994' ) === 'done' ) return;
	$cur = get_theme_mod( 'moondental_address', '' );
	if ( is_string( $cur ) && strpos( $cur, '(신부동)' ) !== false ) {
		set_theme_mod( 'moondental_address', trim( str_replace( '(신부동)', '', $cur ) ) );
	}
...
function md_autolink_addresses( $html ) {
	...
	$patterns = array(
		...
		'"신부 제5공영주차장"'                                => $park5_url,
		'천안시 동남구 먹거리1길 10'                           => $park5_url,
		'동남구 먹거리1길 10'                                 => $park5_url,
		// 본원 (병원 네이버 지도로 연결)
		'병원 지하 기계식 주차장 (SUV 불가)'                   => $hospital_url,
		'병원 지하 기계식 주차장'                              => $hospital_url,
		'본원 지하 기계식 주차장'                              => $hospital_url,
		'병원 지하 기계식'                                    => $hospital_url,
		'천안시 동남구 만남로 52 문타워'                       => $hospital_url,
...
function moondental_template_router( $template ) {
	...
	$map = array(
		...
		/* v3.51 · 직원 전용 재고관리 */
		'재료실'         => 'page-templates/page-supply.php',
