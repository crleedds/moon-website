...
(function () {
  ...
  /* v3.46 · 임플란트센터 · 시스템 색인 필터
   *   목록은 서버에서 이미 전부 렌더되어 있다. JS 는 걸러 보여주는 역할만 하고,
   *   JS 가 없으면 필터 UI 자체가 숨겨진 채 전체 목록이 그대로 노출된다. */
  (function () {
    var archive = document.querySelector('[data-md-implant-archive]');
    if (!archive) return;

    var filters = archive.querySelector('[data-md-implant-filters]');
    var counter = archive.querySelector('[data-md-implant-count]');
    var empty   = archive.querySelector('[data-md-implant-empty]');
    var rows    = Array.prototype.slice.call(archive.querySelectorAll('.md-implant-sys'));
    ...
    /* 무JS 대비로 숨겨 두었던 UI 를 여기서 켠다 */
    filters.hidden = false;
    if (counter) counter.hidden = false;

    function press(group, attr, value) {
      var btns = filters.querySelectorAll('[data-md-' + group + ']');
    ...
    function apply() {
      ...
      rows.forEach(function (row) {
        var okOrigin = origin === 'all' || row.getAttribute('data-origin') === origin;
        var eras     = (' ' + (row.getAttribute('data-eras') || '') + ' ');
        var okEra    = era === 'all' || eras.indexOf(' ' + era + ' ') !== -1;
        var visible  = okOrigin && okEra;
        row.hidden = !visible;
        if (visible) shown++;
      });
    ...
    filters.addEventListener('click', function (e) {
      ...
      if (btn.hasAttribute('data-md-origin')) {
        origin = btn.getAttribute('data-md-origin');
        press('origin', 'data-md-origin', origin);
      } else if (btn.hasAttribute('data-md-era')) {
        era = btn.getAttribute('data-md-era');
        press('era', 'data-md-era', era);
      } else {
        return;
      }
      apply();
